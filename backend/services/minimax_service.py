...
"""
MiniMax API Service for theological content analysis
Provides key themes and thought questions using MiniMax M1-40K with function calling
"""
...
class TheologicalAnalysis:
    """Structured output from MiniMax analysis"""
    key_themes: List[str]
    thought_questions: List[str]

class MiniMaxService:
    """Service for theological content analysis using MiniMax API"""
    
    def __init__(self, api_key: str = None, base_url: str = "https://api.minimax.io/v1"):
        self.api_key = api_key or os.getenv('MINIMAX_API_KEY')
        self.base_url = base_url
        self.model = "MiniMax-M1"  # Correct MiniMax model
        self.timeout = 120.0  # Increased for sermon generation
        self.max_retries = 3
        
        if not self.api_key:
            logger.warning("MiniMax API key not found. Set MINIMAX_API_KEY environment variable.")
    
    async def generate_sermon(self, prompt: str) -> str:
        """
        Generate a sermon using MiniMax API
        ...
        """
        if not self.api_key:
            raise ValueError("MiniMax API key is required for sermon generation")
        ...
        payload = {
            ...
            "temperature": 0.7,
            "max_tokens": 8000,  # Increased for sermon generation
            "top_p": 0.9
        }
        
        logger.info(f"Generating sermon with MiniMax API...")
        logger.debug(f"Prompt length: {len(prompt)} characters")
        
        # Use longer timeout for sermon generation
        sermon_timeout = 300.0  # 5 minutes for sermon generation
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if 'choices' in result and len(result['choices']) > 0:
                            ...
                            logger.error(f"No choices in MiniMax response: {result}")
                            raise ValueError("No sermon content generated")
                    
                    else:
                        logger.error(f"MiniMax API error: {response.status_code} - {response.text}")
                        if attempt == self.max_retries - 1:
                            raise ValueError(f"MiniMax API failed after {self.max_retries} attempts")
                        
                        await asyncio.sleep(2 ** attempt)  # Exponential backoff
                        
                except httpx.TimeoutException:
                    logger.error(f"MiniMax API timeout on attempt {attempt + 1}")
                    if attempt == self.max_retries - 1:
                        raise ValueError("MiniMax API timed out")
    ...
    def _create_system_prompt(self) -> str:
        ...
        return """You are a theological content analyzer for pastors and Bible teachers.

Extract exactly 3 key themes and 2 reflection questions from the content. Focus on biblical insights and practical application.
...
Call the analyze_theological_content function with your analysis."""
    ...
    def _create_user_prompt(self, content: str, title: str = None, category: str = None) -> str:
        ...
        # Skip title if it's already in the content to avoid duplication
    ...
    async def analyze_content(self, content: str, title: str = None, category: str = None) -> TheologicalAnalysis:
        """
        Analyze theological content using MiniMax API with function calling
        ...
        try:
            # Prepare the API request (MiniMax format)
            messages = [
            ...
            # Use function calling with MiniMax
            payload = {
                "model": self.model,
                "messages": messages,
                "temperature": 0.7,
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                # Parse response
                result = response.json()
                logger.info(f"MiniMax API full response: {json.dumps(result, indent=2)}")
    ...
    async def generate_chat_response(self, prompt: str) -> str:
        """
        Generate a chat response using MiniMax API (shorter responses for conversation)
        ...
        """
        if not self.api_key:
            raise ValueError("MiniMax API key is required for chat generation")
        ...
        payload = {
            ...
            "temperature": 0.7,
            "max_tokens": 1000,  # For chat responses
            "top_p": 0.9
        }
        
        logger.info(f"Generating chat response with MiniMax API...")
        logger.debug(f"Prompt length: {len(prompt)} characters")
        
        # Use shorter timeout for chat
        chat_timeout = 120.0  # 120 seconds for chat responses
